...
# 코드에 API 키가 노출되지 않습니다. Streamlit Cloud의 [Secrets] 설정에 키를 입력하세요.
# =====================================================================
try:
    TEACHER_API_KEY = st.secrets["KOREAN_API_KEY"]
except Exception:
    TEACHER_API_KEY = None  # 금고에 키가 없으면 None 상태로 대기
...
def check_dict_api(word):
    ...
    try:
        ...
        for item in root.findall('.//item'):
            ...
            if "피동사" in def_text:
                pos_tags.append("피동사")
            elif "사동사" in def_text:
                pos_tags.append("사동사")
            elif pos_text:
                pos_tags.append(pos_text)
...
def upload_png_to_github(img_bytes, student_id, student_name):
    ...
    try:
        req = urllib.request.Request(url, headers=headers, data=json.dumps(data).encode('utf-8'), method='PUT')
        with urllib.request.urlopen(req) as response:
            if response.status in [200, 201]:
                return True, f"✨ 제출 성공! 선생님의 [과제 확인 폴더]로 숙제 파일이 안전하게 전송되었습니다. 고생했어요!"
    except Exception as e:
        return False, f"❌ 폴더로 전송하는 중에 오류가 났어요. 선생님께 문의해 주세요! ({str(e)})"


# --- 3. 정밀 문법 분석 함수 ---
def analyze_sentence(text, target_type):
    # 🚨 금고에 API 키가 등록되지 않았을 때의 안전 차단문
    ...
    while i < len(tokens):
        ...
        if i < len(tokens) - 1 and tag == 'EC' and form in ['어', '아'] and tokens[i + 1].tag == 'VX' and tokens[
            i + 1].form == '지':
            if target_type == 2: found_target = True
            j_vowel = "아" if is_yang_vowel(last_verb_stem) else "어"
            jida_form = f"-{j_vowel}지다"
            display_html.append(PASSIVE_COLOR.format(text=f"{jida_form}(피동)"));
            display_plain.append(f"[{jida_form}(피동)]")
            i += 2;
            continue
        ...
        if tag.startswith('V') and form.endswith('지') and len(form) >= 2:
            ...
            display_html.append(root);
            display_plain.append(root)
            display_html.append(PASSIVE_COLOR.format(text=f"{jtext}(피동)"));
            display_plain.append(f"[{jtext}(피동)]")
            bword = form + "다"
        ...
        if tag.startswith('V') and len(form) >= 2 and form[-1] in ['이', '히', '리', '기']:
            ...
            display_html.append(f"{root}-");
            display_plain.append(f"{root}-")
            bword = form + "다"
            base_forms.append({"word": bword, "valid": check_dict_api(bword)})

            display_html.append(PASSIVE_COLOR.format(text=f"-{suffix}-(피동 접사)"));
            display_plain.append(f"[-{suffix}-(피동 접사)]")
            i += 1;
            continue

        if form in ['"', '“', '”']: display_html.append(DIRECT_COLOR.format(text=form)); display_plain.append(
            f"[{form}]"); i += 1; continue
        if tag == 'JKQ' and form in ['라고', '이라고', '하고'] and has_quotes:
            ...
            display_plain.append(f"[{form}(직접 인용)]");
            i += 1;
            continue
        ...
        if tag.startswith('V'):
            display_html.append(f"{form}-"); display_plain.append(f"{form}-")
        elif tag == 'EP':
            display_html.append(f"-{form}-"); display_plain.append(f"-{form}-")
        elif tag == 'EF':
            display_html.append(f"-{form}"); display_plain.append(f"-{form}")
    ...
    if found_target and base_forms:
        for b in base_forms:
            ...
            if not plist:
                api_rejected = True;
                error_msg = f"❌ '{b['word']}'는 표준국어대사전에 없는 단어예요!";
                break

            if target_type == 1 and plist:
                if '피동사' not in plist and '사동사' in plist:
                    api_rejected = True;
                    error_msg = f"❌ '{b['word']}'는 누군가에게 시키는 '사동사'예요. 당하는 '피동 표현'으로 다시 쓰세요!";
                    break
    ...
    if api_rejected:
        ...
        if target_type == 1:
            error_msg = "❌ [-이/히/리/기-] 피동 접사가 포함된 피동사가 없습니다!"
        elif target_type == 2:
            error_msg = "❌ [-아/어지다] 또는 [-되다] 피동 표현이 없습니다."
        elif target_type == 3:
            error_msg = "❌ 큰따옴표가 포함된 직접 인용 표현이 없습니다."
        elif target_type == 4:
            error_msg = "❌ 조사 '-고'가 쓰인 간접 인용 표현이 없습니다. (큰따옴표 삭제)"

    for b in base_forms:
        if isinstance(b['valid'], list):
            if target_type == 1:
                b['valid'] = ('피동사' in b['valid'])
            else:
                b['valid'] = len(b['valid']) > 0
...
def render_base_form_links(bases):
    ...
    for b in bases:
        ...
        links.append(
            f"<a href='https://stdict.korean.go.kr/search/searchResult.do?pageSize=10&searchKeyword={urllib.parse.quote(w)}' target='_blank' style='text-decoration:none;'>🔍 <b>{w}</b></a> {status}")
    return "👉 [단어 사전에서 보기]: " + " | ".join(links)


# 💡 [두부 박스(ㅁ) 완벽 수리 구간] PNG 생성기
def create_report_png(student_id, student_name, sentences, plain_results, base_forms_list):
    ...
    y_pos = 150
    for idx in range(4):
        draw.text((40, y_pos), labels[idx], fill=(0, 102, 204), font=fsub);
        y_pos += 26
        draw.text((60, y_pos), f"작성 문장: {sentences[idx]}", fill=(0, 0, 0), font=fbody);
        y_pos += 22
        draw.text((60, y_pos), f"형태소 분석: {plain_results[idx]}", fill=(220, 50, 50), font=fbody)

        bases = base_forms_list[idx]
        if bases:
            # ⚠️ 이모지를 배제하고 순수 한글 텍스트 '[확인완료]' 할당 -> ㅁ 깨짐 100% 방지
            btexts = [f"{b['word']} {'[확인완료]' if b['valid'] else '[미등재/오답]'}" for b in bases]
            y_pos += 22;
            draw.text((60, y_pos), f"▶ 기본형: " + ", ".join(btexts), fill=(0, 120, 50), font=fbody)
        y_pos += 45
...
c1, b1 = st.columns([85, 15]);
c1.text_input("1️⃣ 피동 표현 (-이-, -히-, -리-, -기-)", key="q1", placeholder="예: 토끼가 사자에게 발목을 잡혔다.");
b1.write("");
b1.write("");
b1.button("🔄 지우기", key="btn1", on_click=clear_q, args=("q1",))
c2, b2 = st.columns([85, 15]);
c2.text_input("2️⃣ 피동 표현 (-아/어지다 또는 -되다)", key="q2", placeholder="예: 책상 위에 있던 우유가 쏟아졌다.");
b2.write("");
b2.write("");
b2.button("🔄 지우기", key="btn2", on_click=clear_q, args=("q2",))
c3, b3 = st.columns([85, 15]);
c3.text_input("3️⃣ 직접 인용 표현", key="q3", placeholder="예: 민수가 나에게 \"매점 가자.\"라고 말했다.");
b3.write("");
b3.write("");
b3.button("🔄 지우기", key="btn3", on_click=clear_q, args=("q3",))
c4, b4 = st.columns([85, 15]);
c4.text_input("4️⃣ 간접 인용 표현", key="q4", placeholder="예: 선생님께서 내일 체육복을 입고 오라고 하셨다.");
b4.write("");
b4.write("");
b4.button("🔄 지우기", key="btn4", on_click=clear_q, args=("q4",))

st.markdown("---")

if st.button("🚀 내 과제 채점해보기"):
    if not s_id.strip() or not s_name.strip():
        st.error("⚠️ 학번과 이름을 먼저 입력해 주세요!")
    elif not (st.session_state.q1 and st.session_state.q2 and st.session_state.q3 and st.session_state.q4):
        st.error("⚠️ 4개 문항 중 아직 안 쓴 칸이 있어요.")
    else:
        ...
        if ok1 and ok2 and ok3 and ok4:
            ...
            png_data = create_report_png(s_id, s_name, [st.session_state.q1, st.session_state.q2, st.session_state.q3,
                                                        st.session_state.q4], [p1, p2, p3, p4], [b1, b2, b3, b4])
